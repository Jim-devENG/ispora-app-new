...
app.get("/make-server-b8526fa6/sessions", async (c) => {
  try {
    ...
    const userSessions = (Array.isArray(allSessions) ? allSessions : [])
      .filter((s: any) => s.mentorId === user.id || s.studentId === user.id)
      .sort((a: any, b: any) => new Date(a.scheduledAt).getTime() - new Date(b.scheduledAt).getTime());

    // Enrich with mentorship and user data
    const enrichedSessions = await Promise.all(
      userSessions.map(async (session: any) => {
        const mentorship = await kv.get(`mentorship:${session.mentorshipId}`);
        const student = await kv.get(`user:${session.studentId}`);
...
app.post("/make-server-b8526fa6/sessions/:sessionId/register", async (c) => {
  try {
    ...
    await kv.set(`notification:${notificationId}`, {
      ...
      message: `${user.firstName} ${user.lastName} registered for your public session`,
      read: false,
      data: { sessionId },
      createdAt: new Date().toISOString(),
    });
...
// ══════════════════════════════════════════════════════════════════════════════
// ── SHORT CODE RESOLUTION (NO AUTH REQUIRED) ──
// ══════════════════════════════════════════════════════════════════════════════
// NOTE: This MUST be LAST to avoid catching other routes like /resources
